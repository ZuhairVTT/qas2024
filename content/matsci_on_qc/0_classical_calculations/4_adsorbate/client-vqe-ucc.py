        ...
        class CustomAdaptVQE(StatefulAdaptVQE):
            ...
            def solve(self, problem):
                try:
                    ...
                    if not hasattr(result, 'total_energies') or not result.total_energies:
                        logger.warning("Invalid result: no total_energies found")
                        return result

                    current_energy = result.total_energies[-1]
                    self._eval_count += 1
                    
                    # Check for alternating pattern
                    if self._last_energy is not None:
                        if abs(current_energy - self._last_energy) < 1e-3:
                    ...
                    self.previous_energies.append(current_energy)
                    ...
                    if len(self.previous_energies) == self.convergence_window:
                        ...
                        if energy_range < self.convergence_threshold or self._alternating_count >= 3:
                            logger.info(f"ADAPT-VQE Convergence achieved! Energy range: {energy_range}")
